 ...
 */
export function resolveOpenRouterEnv(
  env: NodeJS.ProcessEnv = process.env,
): OpenRouterEnvConfig {
  const freeModel =
    env.OPENROUTER_FREE_MODEL?.trim() || OPENROUTER_FREE_ROUTER;
  const preferFree =
    env.OPENROUTER_PREFER_FREE === "1" ||
  ...
  const providerSort =
    sortRaw === "price" || sortRaw === "throughput" || sortRaw === "latency"
      ? sortRaw
      : undefined;
 ...
 */
export function createOpenRouterInferenceClient(
  options: OpenRouterInferenceOptions,
): InferenceClient {
  ...
  const lowComputeModel =
    options.lowComputeModel || options.defaultModel || OPENROUTER_FREE_ROUTER;
  let currentModel = defaultModel;
  let maxTokens = options.maxTokens ?? 4096;
  const siteUrl = options.siteUrl || "https://solanaclawd.com";
  const appName = options.appName || "solanaclawd";
...
export function createOpenRouterFromEnv(
  env: NodeJS.ProcessEnv = process.env,
  overrides?: Partial<OpenRouterInferenceOptions>,
): InferenceClient | null {
  const resolved = resolveOpenRouterEnv(env);
  if (!resolved.apiKey) return null;

  const provider: OpenRouterProviderPrefs | undefined = resolved.providerSort
